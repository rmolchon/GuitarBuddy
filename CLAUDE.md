...
GuitarBuddy/
  ...
  Core/
    Audio/                 # PitchDetectorProtocol, PitchDetector (YIN), AudioEngineController, AudioFrame, RecordPermissionProviding,
...
- Capture via `AVAudioEngine.inputNode.installTap`. The tap block does the minimum on the real-time audio render thread — copy the buffer into a value-type `AudioFrame` (`samples: [Float]` + `sampleRate`) via `AudioEngineController.frame(from:)` — then hands off to a private serial `processingQueue` (`.userInitiated`) where the heavy YIN pitch detection runs. `frameStream` publishes `AudioFrame` snapshots, not the live `AVAudioPCMBuffer` (which is only valid for the duration of the tap callback — an async consumer reading it later is a use-after-free). Both `frameStream` and `pitchStream` use `.bufferingNewest(N)` buffering (`frameBufferDepth`/`pitchBufferDepth` constants) so a backed-up consumer drops the oldest frames instead of growing an unbounded backlog.
- `AudioEngineController.start()` is `async` and **idempotent + transactional**: a second `start()` while running is a no-op (guarded by an `NSLock`-protected `isRunning`), and any failure mid-startup runs `performCleanup(dueTo:)` so a failed `start()` leaves the controller exactly as `stop()` would (tap removed, session inactive, both streams finished). `stop()` is likewise idempotent and safe to call before `start()` or twice. It awaits mic permission, runs a cheap pre-flight `outputFormat(forBus:0)` check, *then* configures the `AVAudioSession` (`.playAndRecord`/`.measurement`) — after which it **re-reads the input format post-activation** and passes *that* to `installTap` (on a real device the format isn't trustworthy until the session is active; a stale format crashes `installTap` with a hardware-format-mismatch assertion). The pre-flight check deliberately runs before touching `AVAudioSession`/`engine.start()` — reaching those with no real input route (as in the XCTest host) crashes the process (`SIGKILL`) rather than throwing a catchable error, so the cheap format guard must short-circuit first. Permission state is read through the injectable `RecordPermissionProviding` protocol (`SystemRecordPermissionProvider` in production) rather than calling `AVAudioApplication` directly, since querying/requesting *live* system permission state is non-deterministic in a headless XCTest host — an undetermined permission hangs awaiting a system prompt that never appears. Tests inject `FakeRecordPermissionProvider` instead.
- **Interruptions fail clean, they don't auto-recover.** `AudioEngineController` observes `AVAudioSession.interruptionNotification` (`.began`), `AVAudioSession.routeChangeNotification` (`.oldDeviceUnavailable`), and `.AVAudioEngineConfigurationChange`; any of them tears the session down via `performCleanup(dueTo: .interrupted)` and sets `terminationError`. `AudioTranscriptionStreamBuilder` checks `terminationError` after its raw-stream loop ends and calls `continuation.finish(throwing:)` instead of a silent `finish()`, so Hum Mode / Transcribe surface the error; `TunerViewModel` does the same after its `pitchStream` loop and also resets `isListening`. A normal user `stop()` leaves `terminationError` nil. `deinit` also runs cleanup as a backstop.
...
- `StrumTranscriptionService` is the second concrete `AudioTranscriptionService`. It's the first (and so far only) consumer of `AudioEngineController.frameStream` (as opposed to `pitchStream`) — it reads each `AudioFrame`'s `samples`/`sampleRate` directly, runs them through `ChromaExtractor`, then `StrumSegmenter`. Like `HumTranscriptionService`, it constructs a fresh `AudioEngineController` per `transcribe()` call for the same single-use-`AsyncStream` reason.
...
The design point that made this work without rework: `AudioEngineController` exposes captured audio as a generic `AsyncStream<AudioFrame>` (`frameStream`) *and* a derived `AsyncStream<Double?>` of YIN-detected pitch (`pitchStream`), rather than being hard-wired to feed only the Tuner's `PitchDetecting` consumer. `HumTranscriptionService` only needed `pitchStream`; `StrumTranscriptionService` needed the raw `frameStream` since chord detection is polyphonic (see "Transcribe" above). Both conformers construct a *fresh* `AudioEngineController` per `transcribe()` call (via an injectable factory) rather than reusing one held for the service's lifetime — `AudioEngineController`'s `AsyncStream`s are single-use, so reusing one instance across a Stop→Start session would silently yield zero events forever (caught via code review while building Hum Mode).
...
- Unit tests (`GuitarBuddyTests`) target ≥80% coverage of `Core/`:
  ...
  - `AudioEngineControllerTests` — covers `samples(from:)`/`frame(from:)` buffer extraction, `handle(_ frame:)` yielding on `frameStream`/`pitchStream`, the `RecordPermissionProviding` branches (`permissionDenied` when already denied, and when an undetermined request is declined), and the reliability contract: a failed `start()` sets `terminationError` and finishes both streams, `stop()` is a safe no-op before `start()` and idempotent when repeated, and a posted `AVAudioSession.interruptionNotification` (`.began`) tears the session down with `terminationError == .interrupted`.
...
- Integration: buffer-conversion/detection-invocation logic lives outside the `installTap` closure (`frame(from:)` + `handle(_ frame:)`) so `AudioEngineController` can be exercised with synthetic frames, no live mic needed.
