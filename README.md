...
- **Key Finder** — build a chord sequence by tapping a root note and quality onscreen (e.g. `C`, `Am`, `F`, `G`), get a best-guess musical key with a confidence score using diatonic chord matching, and hit Play to hear the sequence played back as synthesized chord tones.
